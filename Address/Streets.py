...
URL_STREETS = '/realty.address.streets'
...
class Districts(YuCRM):
    ...
    def __full_url(self):
        return f'{super().url.format(login=super().login, token=super().token)}{URL_STREETS}'

    def List(self,
             id=None, created_at=None, updated_at=None, name=None, city=None, page=None, limit=None, mode=None,
        ...
        params = {'page': page, 'limit': limit, 'mode': mode, 'order_by': order_by, 'order_dir': order_dir, 'id': id,
                  'created_at': created_at, 'updated_at': updated_at, 'name': name, 'city': city}
